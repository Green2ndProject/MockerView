...
function connect() {
    ...
    stompClient.connect({}, function(frame) {
        ...
        stompClient.subscribe('/topic/session/' + sessionId + '/feedback', function(message) {
            const feedback = JSON.parse(message.body);
            displayAIFeedback(feedback);
        });
        
        stompClient.subscribe('/topic/session/' + sessionId + '/interviewer-feedback', function(message) {
            const feedback = JSON.parse(message.body);
...
function sendQuestion() {
    ...
    stompClient.send('/app/session/' + sessionId + '/question', {}, JSON.stringify(questionMessage));
...
function submitAnswer() {
    ...
    stompClient.send('/app/session/' + sessionId + '/answer', {}, JSON.stringify(answerMessage));
...
function submitFeedbackForAnswer(answerId) {
    ...
    stompClient.send('/app/session/' + sessionId + '/interviewer-feedback', {}, JSON.stringify(feedbackMessage));
...
function displayAIFeedback(feedback) {
    ...
    feedbackElement.innerHTML = `
        <div class="ai-feedback">
            <div class="ai-feedback-title">🤖 AI 피드백 #${feedback.answerId}</div>
            <div class="feedback-content">
                ...
                <div class="feedback-meta">
                    <small class="text-muted">
                        ${feedback.model} | ${formatDateTime(feedback.timestamp)}
                    </small>
                </div>
            </div>
        </div>
