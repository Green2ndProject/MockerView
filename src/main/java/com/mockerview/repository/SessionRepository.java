...
public interface SessionRepository extends JpaRepository<Session, Long> {
        ...
        @Query(value = "SELECT s FROM Session s LEFT JOIN FETCH s.host " +
                ...
                "AND (:status IS NULL OR s.sessionStatus = :status) ", countQuery = "SELECT COUNT(s) FROM Session s WHERE s.isSelfInterview = 'N' " +
                        "AND (:keyword IS NULL OR :keyword = '' OR s.title LIKE %:keyword%) " +
        ...
        @Query(value = "SELECT s FROM Session s LEFT JOIN FETCH s.host WHERE s.isSelfInterview = 'N'", countQuery = "SELECT COUNT(s) FROM Session s WHERE s.isSelfInterview = 'N'")
        ...
        List<Session> findByStatusAndIsReviewable(@Param("status") Session.SessionStatus status,
        ...
        @Query(value = "SELECT s FROM Session s LEFT JOIN FETCH s.host " +
                "WHERE s.isSelfInterview = 'N' AND s.sessionStatus = :status", countQuery = "SELECT COUNT(s) FROM Session s " +
        ...
        @Query(value = "SELECT s FROM Session s LEFT JOIN FETCH s.host " +
                ...
                "ORDER BY s.endTime DESC", countQuery = "SELECT COUNT(s) FROM Session s " +
                        "WHERE s.sessionStatus = :status AND s.isReviewable = :isReviewable")
        ...
        @Query(value = "SELECT s FROM Session s LEFT JOIN FETCH s.host " +
                ...
                "ORDER BY s.createdAt DESC", countQuery = "SELECT COUNT(s) FROM Session s " +
                        "WHERE s.host.id = :hostId AND s.isSelfInterview = :isSelfInterview")
        ...
        @Query("SELECT DISTINCT s FROM Session s LEFT JOIN FETCH s.host LEFT JOIN FETCH s.questions WHERE s.host.id = :hostId AND s.isSelfInterview = 'Y' ORDER BY s.createdAt DESC")
        List<Session> findSelfInterviewsByHostId(@Param("hostId") Long hostId);

        @Query(value = "SELECT DISTINCT s FROM Session s LEFT JOIN FETCH s.host LEFT JOIN FETCH s.questions WHERE s.host.id = :hostId AND s.isSelfInterview = 'Y'", 
                countQuery = "SELECT COUNT(s) FROM Session s WHERE s.host.id = :hostId AND s.isSelfInterview = 'Y'")
        Page<Session> findSelfInterviewsByHostIdPageable(@Param("hostId") Long hostId, Pageable pageable);
        ...
        Long countBySessionStatusAndIsSelfInterview(@Param("status") SessionStatus status,
                @Param("isSelfInterview") String isSelfInterview);

        @Query("SELECT s FROM Session s LEFT JOIN FETCH s.host WHERE s.host = :host AND s.isSelfInterview = :isSelfInterview ORDER BY s.createdAt DESC")
        List<Session> findByHostAndIsSelfInterviewOrderByCreatedAtDesc(@Param("host") User host,
                @Param("isSelfInterview") String isSelfInterview);

        @Query("SELECT s FROM Session s LEFT JOIN FETCH s.host WHERE s.isSelfInterview = :isSelfInterview ORDER BY s.createdAt DESC")
        List<Session> findByIsSelfInterviewOrderByCreatedAtDesc(@Param("isSelfInterview") String isSelfInterview);
